namespace Endabgabe {
    ...
    let imagedata: ImageData; //malen
    let fps: number = 25; //frames per second
    let i: number = 0; 
    let xMouse: number; //welche x position die maus hin klickt (wegen dem Schneeball)
    let yMouse: number; //same as x nur mit  y koordinate
    ...
    function startGame(): void {
        ...
        imagedata = crc2.getImageData(0, 0, canvas.width, canvas.height); //getimagedata -> das es malt
    ...
    //Schneeball
    function generateSnowball(_xMouse: number, _yMouse: number): void { //_ = übergabeparameter und wird beim klicken generiert
    ...
    function gameEnds(): void {
        ...
        ende.classList.remove("canvassize");
        let yourScore: HTMLElement = <HTMLElement>document.getElementById("yourScore");
        innerText = "Deine Punktzahl:" + " " + score.toString();
