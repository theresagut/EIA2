namespace Firework {
  ...
  let serverPage: string = "meinLinkmuss rein"; //"http://localhost:5001/";
  ...
  function createObject(_event: MouseEvent): void {
    ...
    let mousePositionX: number = _event.offsetX; //- crc2.canvas.offsetLeft;
    let mousepositionY: number = _event.offsetY; //- crc2.canvas.offsetTop;
    let formData: any = new FormData(document.forms[0]);
  ...
  function createParticle(direction: string, _quantity: number, _mousePositionX: number, _mousePositionY: number, _color: string, _lifetime: number, _type: string): void {
